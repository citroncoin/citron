...
//
// Bootup the masternode, look for a 250000 CTRN input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Citron.conf */
...
{
    CCitronAddress address(collateralAddress);
