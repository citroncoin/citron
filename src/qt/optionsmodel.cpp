...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCitronAmount"))
        settings.setValue("nAnonymizeCitronAmount", 1000);
    nAnonymizeCitronAmount = settings.value("nAnonymizeCitronAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCitronAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCitronAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCitronAmount:
            return QVariant(nAnonymizeCitronAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCitronAmount:
            nAnonymizeCitronAmount = value.toInt();
            settings.setValue("nAnonymizeCitronAmount", nAnonymizeCitronAmount);
            emit AnonymizeCitronAmountChanged(nAnonymizeCitronAmount);
            break;
        default:
            break;
        }
