...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCitronAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCitronAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCitronAmount = strAnonymizeCitronAmount.remove(strAnonymizeCitronAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCitronAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCitronAmount*COIN) nMaxToAnonymize = nAnonymizeCitronAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCitronAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCitronAmount));
        strAnonymizeCitronAmount = strAnonymizeCitronAmount.remove(strAnonymizeCitronAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCitronAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
