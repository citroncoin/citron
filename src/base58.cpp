...
namespace {
    class CCitronAddressVisitor : public boost::static_visitor<bool> {
    private:
        CCitronAddress *addr;
    public:
        CCitronAddressVisitor(CCitronAddress *addrIn) : addr(addrIn) { }
...
bool CCitronAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CCitronAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CCitronAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CCitronAddressVisitor(this), dest);
}

bool CCitronAddress::IsValid() const {
...
CTxDestination CCitronAddress::Get() const {
    if (!IsValid())
...
bool CCitronAddress::GetKeyID(CKeyID &keyID) const {
...
bool CCitronAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CCitronSecret::SetKey(const CKey& vchSecret) {
...
CKey CCitronSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CCitronSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CCitronSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CCitronSecret::SetString(const std::string& strSecret) {
